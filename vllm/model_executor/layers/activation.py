...
class XIELU(CustomOp):
    ...
    def __init__(self, alpha_p=0.8, alpha_n=0.8, beta=0.5, eps=-1e-6):
        ...
        # for inference, we can just precompute the restrictions on the values of alpha_p and alpha_n
        self.alpha_p = F.softplus(self.alpha_p)
        self.alpha_n = self.beta + F.softplus(self.alpha_n)  

        if current_platform.is_cuda_alike():
            # cuda implementation under development, using forward_native for now
    ...
    def forward_native(self, x: torch.Tensor) -> torch.Tensor:
        return torch.where(
            ...
            self.alpha_n * torch.expm1(torch.min(x, self.eps)) - self.alpha_n * x + self.beta * x
        )
